...
void GMnElasPeak( double E_beam, double BB_angle, double BB_distance, double Q2, double hcal_angle, double hcal_distance )
{

  hcal_angle *= TMath::Pi()/180.0; // Deg to rad.
  double hcal_minang = hcal_angle - ( (hcal_width/2)/hcal_distance ); //approx with arclength
  double hcal_maxang = hcal_angle + ( (hcal_width/2)/hcal_distance ); //approx with arclength
  ...
  double deltaBBang = 0.;
  BB_angle *= TMath::Pi()/180.0; // Deg to rad.
  double sh_faceDist = 3.1 + BB_distance; //1.2m to GEMs, another 1.9m to BBCal from the BigBite magnet.

  double elasPeak_e[7] = {0.};
  double elasPeak_p[7] = {0.};
  double elasPeak_n[7] = {0.};
  double epsilon_p[7] = {0.};
  double epsilon_n[7] = {0.};
  double nu_p[7] = {0.}; //keeping around in arrays for possible readout in future versions. Just KE where KE = E_beam - E_e'
  double nu_n[7] = {0.};
  double p_p[7] = {0.};
  double p_n[7] = {0.};
  double p_e[7] = {0.};
  ...
  std::cout << "///////////////////////////////////////////////////////////////////////////////////////////////////////////////" << endl;
  std::cout << "//   BBSh col  |  e' angle   |  e' p      |  elas proton KE  |  elas neutron KE |  had proj ang  |  on hcal? //" << endl;

  for(int shcol=0; shcol<7; shcol++){

    effective_BBang[shcol] = (sh_ypos[shcol]/sh_faceDist) + BB_angle;

    //Proton
    double ePeak_e = E_beam/( 1. + (2.*E_beam/M_p)*pow(sin(effective_BBang[shcol]/2.), 2.) );

    elasPeak_e[shcol] = ePeak_e;
    nu_p[shcol] = E_beam-ePeak_e;
    p_p[shcol] = sqrt(pow(nu_p[shcol],2.)+2.*M_p*nu_p[shcol]);

    double phel = sqrt( pow(nu_p[shcol],2.)+2.*M_p*nu_p[shcol] );
