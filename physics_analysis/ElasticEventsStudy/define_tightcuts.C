...
void calc_bbcal_hcal_thresholds(const int kine_num)
{

  lookup_kinematic_info(kine_num);	
  //readin_beamvariables(11494);
...
void define_tightcuts(const char* rootfilename, const int kine_num)
{
	TFile* anarootfile = TFile::Open(rootfilename);

	if (anarootfile)
	{
		gStyle->SetOptFit(1);

		TH1D* h1_bbtrackvertz = static_cast<TH1D*>(anarootfile->Get("h1_bbtrackvertz"));
		if(h1_bbtrackvertz) vertexZ_cut(h1_bbtrackvertz);
		else std::cout << "The histogram h1_bbtrackvertz does not exist.\n";

		TH1D* h1_bbcal_hcal_tdiff = static_cast<TH1D*>(anarootfile->Get("h1_bbcal_hcal_tdiff"));
		if (h1_bbcal_hcal_tdiff) bbCal_HCal_coincut(h1_bbcal_hcal_tdiff);	
		else std::cout << "The histogram h1_bbcal_hcal_tdiff does not exist.\n";

		TH1D* h1_EoverP = static_cast<TH1D*>(anarootfile->Get("h1eprime_EoverP"));
		if (h1_EoverP) e_over_p_cut(h1_EoverP);
		else std::cout << "The histogram h1eprime_EoverP does not exist. \n";

		calc_bbcal_hcal_thresholds(kine_num);

		TH1D* h1_HCal_e = static_cast<TH1D*>(anarootfile->Get("h1HCal_e"));
		if (h1_HCal_e) hcal_energy_cut(h1_HCal_e);
		else std::cout << "The histogram h1HCal_e does not exist. \n";

		TH1D* h1_sh_ps_e = static_cast<TH1D*>(anarootfile->Get("h1_sh_ps_e"));
		if (h1_sh_ps_e) ps_plus_sh_energy_cut(h1_sh_ps_e);
		else std::cout << "The histogram h1_sh_ps_e does not exist. \n";

	}

	else std::cout << "The root file " << rootfilename << "could not be found in the current directory.\n";
